  ...
  const validarDatos = (
    ...
    const penultimo = (fechaDate) => {
      ...
      if (penult !== "") {
        ...
        if (
          fechaDate.slice(0, 1) === "1" &&
          siglo19[fechaDate.charAt(0)].indexOf(penult) >= 0
        )
          sep_siglo = penult;
        // Si la fecha es del siglo XX y el penúltimo dígito es una letra, entonces es válido
        if (
          fechaDate.slice(0, 1) === "2" &&
          siglo20[fechaDate.charAt(0)].indexOf(penult) >= 0
        )
          sep_siglo = penult;
      }
      ...
      if (sep_siglo === "") {
        if (fechaDate.slice(0, 1) === "1") sep_siglo = "0";
        else sep_siglo = "A";
      }
      return sep_siglo;
    };
    ...
    const fechaStr = `${fechaDate.getFullYear().toString().substr(-2)}${(
    ...
    const penult = penultimo(
      `${primerasDosLetrasapellidoP}${primerLetraapellidoM}${primerasDosLetrasNombre}${fechaStr}${genero.toUpperCase()}${estado.toUpperCase()}${consonanteApellidoP}${consonanteApellidoM}${consonanteNombre}${vocalNombre}`
    );
    const homoclave = obtener_ultimo(
      `${primerasDosLetrasapellidoP}${primerLetraapellidoM}${primerasDosLetrasNombre}${fechaStr}${genero.toUpperCase()}${estado.toUpperCase()}${consonanteApellidoP}${consonanteApellidoM}${consonanteNombre}${vocalNombre}${penult}`
    );
