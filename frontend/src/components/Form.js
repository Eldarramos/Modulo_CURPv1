...
const Formulario = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/cities/${idstate}`)// cambie ruta
      .then(response => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/colonies/${idcity}`) // cambie ruta
      .then(response => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/colonies/${idcolony}/postal_code`) // cambie ruta
      .then(response => {
...
//Funcion para guardar datos en db
  ...
  const validarDatos = (nombre, apellidoP, apellidoM, genero, estado, fecha) => {
    ...
    const penultimo = (fecha) =>{
      ...
      if (penult !== ""){
        ...
        if (fecha.getFullYear().toString().slice(0, 2) === "19" && siglo19.indexOf(penult) >= 0){
          sep_siglo = penult;}
        // Si la fecha es del siglo XX y el penúltimo dígito es una letra, entonces es válido
        if (fecha.getFullYear().toString().slice(0, 2) === "20" && siglo20.indexOf(penult) >= 0){
          sep_siglo = penult;}
      }
  ...
  const calcularPenultimoDigito = (fechaDate) => {
    ...
    const fechaStr = `${fechaDate.getFullYear().toString().slice(-2)}${(fechaDate.getMonth() + 1).toString().padStart(2, '0')}${fechaDate.getDate().toString().padStart(2, '0')}`;
...
const validarCurp = (curp) => {
  ...
  {activeStep === 0 && ( 
    <div>
      ...
      <input className=' col-md-6 px-4 my-2' value={nombre} onChange={(e) => setNombre(e.target.value.toUpperCase())} type= 'text'/>
     </div>

     <div className="input-container">
      <label className='label col-md-6 px-4 my-2'>Apellido Paterno:</label>
      <input className=' col-md-6 px-4 my-2' value={apellidoP} onChange={(e) => setApellidoP(e.target.value.toUpperCase())} type= 'text'/>
      </div>

      <div className="input-container">
      <label className='label col-md-6 px-4 my-2'>Apellido Materno:</label>
      <input className='input col-md-6 px-4 my-2' value={apellidoM} onChange={(e) => setApellidoM(e.target.value.toUpperCase())} type= 'text'/>
      </div>

      <div className="input-container">
      <label className="label col-md-6 px-4 my-2">Género:</label>
      <select className='select col-md-6 px-2 my-1' value={genero} onChange={(e) => setGenero(e.target.value)}>
      ...
      <div className= "input-container">
      <label className='label col-md-6 px-4 my-2'>Fecha de nacimiento:</label>
      <DatePicker 
      ...
      <div className= "input-container">
      <label className=" label col-md-6 px-4 my-2">Estado de nacimiento:</label>
      <select className='select col-md-6 px-2 my-1' value={estado} onChange={(e) => setestado(e.target.value)} type= 'text'>
      <option value="">--Selecciona una opción--</option>
      ...
      <div className="input-container">
         ...
         <label className='label col-md-6 px-4 my-2'>CURP:</label>
          <input
           ...
           style={{ border: isValid === true ? '2px solid green' : '2px solid red' }}/>
           {mensajeError ? <p className='error'>{mensajeError}</p> : null} 
           {isValid === true && <p style={{ color: 'green' }}>CURP válida</p>}
           {isValid === false && <p style={{ color: 'red' }}>CURP no válida</p>}
              ...
              if (curp === curpF) {
               ...
               if (existeCurp) {
                setMensajeError('La CURP ya existe en la base de datos')
                 ...
                 setMensajeError(<span className="mensaje-verde">Registro Valido</span>);
                   }
                 } else {
                 setIsValid(false);
                        }
  ...
  {activeStep === 1 && (
    <div>
     ...
     <div className="input-container">
      ...
      <select className='select col-md-6 px-2 my-1' value={pais} onChange={(e) => setPais(e.target.value)} type= 'text'>
      <option value="">--Selecciona una opción--</option>
      ...
      <select className='select col-md-6 px-1 my-1' value={idstate}  onChange={handleStateChange}>
        ...
        <select className='select col-md-6 px-4 my-2' value={idcity} onChange={handleCityChange}>
         ...
         <div className="input-container">
          <label className='label col-md-6 px-4 my-2'>Colonia:</label>
           <select className='select col-md-6 px-4 my-2' value={idcolony} onChange={handleColonyChange}>
        ...
        <div className="input-container">
         ...
         <input className='input col-md-6 px-4 my-2' value={calle} onChange={(e) => setCalle(e.target.value)} type= 'text'/>
           </div>

        <div className="input-container">
         <label className='label col-md-6 px-4 my-2'>Numero:</label>
         <input className='input col-md-6 px-4 my-2' value={numero} onChange={(e) => setNumero(e.target.value)} type= 'text'/>
           </div>

        <div className="input-container">
          <label className='label col-md-6 px-4 my-2'>Codigo Postal:</label>
          <select className='select col-md-6 px-4 my-2' value={postal_code} onChange={handleCodigoChange}>
    ...
    <div>
      <div className="input-container">
          <label className='label col-md-6 px-4 my-2'>Teléfono:</label>
            <input className='input col-md-6 px-4 my-2' value={telefono} onChange={(e) => setTelefono(e.target.value)} type= 'text'/>
        </div>

         <div className="input-container">
            <label className='label col-md-6 px-4 my-2'>Celular:</label>
            <input className='input col-md-6 px-4 my-2' value={celular} onChange={(e) => setCelular(e.target.value)} type= 'text'/>
          </div>
     
        <div className="input-container">
          <label className='label col-md-6 px-4 my-2'>Escuela:</label>
           <input className='input col-md-6 px-4 my-2' value={escuela} onChange={(e) => setEscuela(e.target.value)} type= 'text'/>
          </div>

          <div className="input-container">
           <label className='label col-md-6 px-4 my-2'>Trabajo:</label>
           <input className='input col-md-6 px-4 my-2' value={trabajo} onChange={(e) => setTrabajo(e.target.value)} type= 'text'/>
          </div>
